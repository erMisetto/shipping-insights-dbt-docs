        ...
        create or replace transient table BUSINESS_INT_DBT.PUBLIC_marts.mart_container_trace
         as
        (select * from (
...
from enriched_trace
              ) order by (shipment_unique_key)
        );
      alter  table BUSINESS_INT_DBT.PUBLIC_marts.mart_container_trace cluster by (shipment_unique_key);
