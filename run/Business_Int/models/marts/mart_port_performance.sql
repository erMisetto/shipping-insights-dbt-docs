        ...
        create or replace transient table BUSINESS_INT_DBT.PUBLIC_marts.mart_port_performance
         as
        (select * from (
...
join port_info p on t.port_code = p.port_code
              ) order by (port_code, snapshot_quarter)
        );
      alter  table BUSINESS_INT_DBT.PUBLIC_marts.mart_port_performance cluster by (port_code, snapshot_quarter);
