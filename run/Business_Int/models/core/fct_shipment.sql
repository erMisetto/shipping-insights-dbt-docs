        ...
        create or replace transient table BUSINESS_INT_DBT.PUBLIC_core.fct_shipment
         as
        (select * from (
...
from src
              ) order by (departure_port_code, leg_departure_date)
        );
      alter  table BUSINESS_INT_DBT.PUBLIC_core.fct_shipment cluster by (departure_port_code, leg_departure_date);
